...
| Configuration studio | Live custom fields (add/archive), leave policies CRUD, payroll rule sets, role management, integration status, REST info |
...
1. **Wire admin payroll page** — show the seeded Jun 1–15 run (lines,
   exceptions, totals) instead of mock; approve/submit transitions.
2. **Convert-to-employee** — one action on a hired candidate creating the
   worker (entity/position/manager/start from the offer), closing the loop
   recruiting → directory → payroll → PTO.
3. **Wire compliance page** — live packs, statuses, and the audit trail feed.
4. **Legal entities CRUD** — small; unblocks real multi-entity setup.
5. **Workflows: run list live** — read side first; builder persistence later.
6. **Analytics v1** — headcount trend, dept mix, leave usage, attendance hours
   from live tables.
7. **Performance module** — review cycles, goals (referenced in role brief;
   not yet modeled).
8. **Integrations hardening** — real Resend key, Time Doctor field-test,
